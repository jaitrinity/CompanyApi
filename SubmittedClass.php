...
class SubmittedClass{
	public function submitted($empId, $roleId){
		...
		$historySql = "Select"
					...
					." l.Name as locationName,a.MobileDateTime as SubmitDateTime,a.GeoLocation,"
					...
					." where a.Event = 'Submit' and a.MobileDateTime >= DATE_SUB(CURDATE(), INTERVAL  90 DAY)" 
		...
		if($historySize > 0){
			while($hRow = mysqli_fetch_array($historyQuery)){
				...
				if($sub == '' && $caption == ''){
					...
					$hObj->geoLocation = $hRow['GeoLocation'];
					$hObj->Caption = $cat;
					$hObj->Icon = $iconArr[0];
					$hObj->subCategoryList = array();
					
					$this->getFilledCheckpoints($hObj,$statusObj);
				}
				else if($sub != '' && $caption == ''){
					...
					$hObj1->geoLocation = $hRow['GeoLocation'];
					$hObj1->Caption = $sub;
					$hObj1->Icon = $iconArr[1];
					$hObj1->subCategoryList = array();

					$this->getFilledCheckpoints($hObj1,$statusObj);
					...
					$hObj2->geoLocation = $hRow['GeoLocation'];
					$hObj2->Caption = $caption;
					$hObj2->Icon = $iconArr[2];
					$hObj2->subCategoryList = array();

					$this->getFilledCheckpoints($hObj2,$statusObj);
