...
class NewAssign1{
	public function getNewAssign1Data($empId){
		...
		$geofenceSql = "SELECT `Geofence` FROM `Configration`";
		...
		$verifierSql = "Select mp.ActivityId,mp.MenuId,mp.LocationId,l.Name,l.GeoCoordinates,m.Verifier,mp.Start,mp.End,
						m.Caption,m.Sub,m.Cat,m.Icons,m.CheckpointId,m.GeoFence
						from Mapping mp
		...
		while($ap = mysqli_fetch_assoc($approverQuery)){
			...
			if($sub == '' && $caption == ''){
				...
				$actObj->level = 3;
				$this->getVerifierCheckpoints($actObj,$apObj2);
	...
	function getApproverCheckpoints($cpObj,$apObj){
		...
		$approverActId = $cpObj->verifierActId;
